...
`meridian-net-proxy-pipe` is centered on networking work in TypeScript. The technical target is: Design a TypeScript verification harness for proxy systems, covering stream reduction, windowed input fixtures, and failure-oriented tests.
...
The score is deterministic. The current threshold is 176. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 4.
